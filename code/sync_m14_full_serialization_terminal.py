...
for r in rows:
    if r['family_id'] == 'F14':
        r['status'] = 'IDECAMB_K1_K2_PASS_K3_SCOPED_K4_FULL_SERIALIZATION_FAIL'
        r['notes'] = ('Original iDM route: K1 pass, signed cdm_c K2, K3 partial, .01/.02 K4 FAIL. '
                      'Independent IDECAMB: CQ execution/theory-output/K1 pass with scope; K2 one-sided beta>=0. '
                      'Parent and smaller-step K4 gates fail. Broyden correction is insufficient. '
                      'Full-serialization run 34467577947 (job 102839655588, artifact 10148183299) also fails the frozen angle gate: '
                      'combined norm mismatch 0.00807 passes 0.10 but angle 7.610 deg exceeds 3 deg; dw/TT/EE remain directionally unstable. '
                      'Serialization precision is excluded as a sufficient explanation. K5 remains blocked; no family falsification.')
...
for r in rows:
    if r['family_id'] == 'F14':
        r['K4_numerical_robustness'] = 'FAIL_LOCAL_CONVERGENCE'
        r['K5_multichannel_rank'] = 'BLOCKED_BY_K4'
        r['coverage_state'] = 'IDECAMB_K1_K2_PASS_K3_SCOPED_K4_FULL_SERIALIZATION_FAIL'
        r['notes'] = ('Two-provider record retained. IDECAMB K1 PASS_WITH_SCOPE and K2 one-sided beta>=0. Parent and smaller-step K4 gates fail. '
                      'Broyden-only correction is insufficient. Full-serialization run 34467577947 with ES25.15E3 output leaves combined tangent '
                      'angle 7.61003 deg >3 deg despite norm mismatch 0.00807 <0.10; channel angles include rho_de 4.032 deg, dw 6.845 deg, '
                      'TT 11.184 deg, EE 53.376 deg. Therefore text serialization is not the cause; K5/K6 remain closed pending independently '
                      'justified branch/initialization resolution.')
with p.open('w', newline='') as f:
    w = csv.DictWriter(f, fieldnames=fields); w.writeheader(); w.writerows(rows)

audit = '''## 2026-09-10 — IDECAMB full-serialization terminal control
...
append_once('models/coupled_quintessence/audit.md', 'IDECAMB full-serialization terminal control', audit)

state = '''## 2026-09-10 — M14 IDECAMB full-serialization control consumed
...
append_once('recovery/STATE.md', 'M14 IDECAMB full-serialization control consumed', state)

restore = '''### Latest M14 IDECAMB diagnostic exclusion (2026-09-10)
...
append_once('recovery/RESTORE_FROM_NEW_CHAT.md', 'Latest M14 IDECAMB diagnostic exclusion', restore)

log = '''## 2026-09-10 — M14 full-serialization control closes output-precision hypothesis
...
append_once('logs/research_log.md', 'M14 full-serialization control closes output-precision hypothesis', log)

meth = '''### Serialization-control lesson (M14 IDECAMB)
...
append_once('protocol/FUTURE_MODEL_CONSTRUCTION_METHODOLOGY_v0.2.md', 'Serialization-control lesson (M14 IDECAMB)', meth)
