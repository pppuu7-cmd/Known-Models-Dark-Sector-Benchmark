...
lane="$1"; PIN='e85808324f51fc694d12e3ed7439552a3c3f9540'; profile="lphase_${lane}.pre"; manifest="lphase_${lane}_manifest.json"
...
python3 verification/m21/build_l_grid_phase_profile.py "$lane" class/cl_permille.pre verification/m21/m21_ncdm_tight.pre "$profile" "$manifest"
...
obj={'schema':'KMDSB.W04.M21.LGridPhaseLane.v0.1','protocol':'protocol/W04_M21_TRANSFER_L_GRID_PHASE_FACTORIAL_v0.1.md','lane':os.environ['LANE'],'l_logstep':m['l_logstep'],'l_linstep':m['l_linstep'],'sparse_l_signature':m['sparse_l_signature'],'provider_head':head,'exact_head':head==os.environ['PIN_ENV'],'case_rc':status,'all_cases_rc0':set(status)==set(cases) and all(v==0 for v in status.values()),'ini_sha256':{c:H('m21_cases/'+c+'.ini') for c in cases},'cl_permille_sha256':H('class/cl_permille.pre'),'ncdm_tight_sha256':H('verification/m21/m21_ncdm_tight.pre'),'profile_sha256':H(os.environ['PROFILE']),'manifest_sha256':H(os.environ['MANIFEST']),'varied_keys':m['varied_keys'],'varied_key_count':m['varied_key_count'],'duplicate_free_serialization':m['duplicate_free_serialization']}
