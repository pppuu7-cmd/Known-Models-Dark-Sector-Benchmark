...
def load(p:Path):
 ...
 if a.ndim!=2 or a.shape[0]<10 or a.shape[1]<8: raise RuntimeError(f'invalid thermodynamics table {p}: {a.shape}')
 return a
...
def xy(a,col,zlo=None,zhi=None):
 z=np.asarray(a[:,1],float); y=np.asarray(a[:,col],float); m=np.isfinite(z)&np.isfinite(y)&(z>=0)
 if zlo is not None: m &= z>=zlo
 if zhi is not None: m &= z<=zhi
 z,y=z[m],y[m]; q=np.argsort(z); z,y=z[q],y[q]; z,idx=np.unique(z,return_index=True); y=y[idx]
 if z.size<5: raise RuntimeError('insufficient thermo z support')
 return z,y
def dist(a,b,col,zlo=500.,zhi=2500.):
 za,ya=xy(a,col,zlo,zhi); zb,yb=xy(b,col,zlo,zhi); lo=max(float(za.min()),float(zb.min())); hi=min(float(za.max()),float(zb.max())); m=(za>=lo)&(za<=hi); za,ya=za[m],ya[m]
 if za.size<5 or hi<=lo: raise RuntimeError('insufficient thermo overlap')
 ybi=np.interp(np.log1p(za),np.log1p(zb),yb); den=max(float(np.linalg.norm(ya)),float(np.linalg.norm(ybi)),1e-300)
 return {'R2_symnorm':float(np.linalg.norm(ya-ybi)/den),'n':int(za.size),'z_min':float(za.min()),'z_max':float(za.max())}
def main(comp:Path,out:Path):
 ...
 missing=sorted(set(LANES)-set(dirs)); unexpected=sorted(set(dirs)-set(LANES)); metas={}; tables={}; identity=not missing and not unexpected
 if identity:
 ...
 else: cls='M21_CMB_BRANCH_NOT_LOCALIZED_IN_PRIMARY_THERMO_STATE_COLUMNS'
 result={'schema':'KMDSB.W04.M21.ThermoStateBranchSignature.v0.1','date':'2026-09-14','protocol':PROTOCOL,'provider':PROVIDER,'classification':cls,'cross_lane_input_identity':bool(identity),'missing_products':missing,'unexpected_products':unexpected,'primary_redshift_window':[500.0,2500.0],'primary_columns':COLS,'branch_change_edges':[a+'__'+b for a,b in CHANGE],'same_branch_control_edges':[a+'__'+b for a,b in CONTROL],'edge_results':edges,'branch_change_localized_count':branch_hits,'same_branch_control_localized_count':control_hits,'lane_state_response':lane_state,'K1_promoted':False,'physical_falsification':False,'interpretation_ceiling':'Thermodynamics state localization only; no code-defect, production setting, global-convergence, K1/K3/K4, or physical M21 claim.'}
 ...
 if cls=='M21_THERMO_STATE_BRANCH_SIGNATURE_BLOCKED': raise SystemExit(1)
if __name__=='__main__':
 if len(sys.argv)!=3: raise SystemExit('usage: thermo_state_branch_signature.py COMPONENTS OUT.json')
