#!/usr/bin/env bash
set -euo pipefail
if [ "$#" -ne 1 ]; then echo "usage: $0 PAIR_ID" >&2; exit 2; fi
pair="$1"; PIN='e85808324f51fc694d12e3ed7439552a3c3f9540'; profile="lsamp_${pair}.pre"; manifest="lsamp_${pair}_manifest.json"
...
python3 verification/m21/build_l_sampling_pair_profile.py "$pair" class/cl_permille.pre verification/m21/m21_ncdm_tight.pre "$profile" "$manifest"
...
PAIR="$pair" PIN_ENV="$PIN" PROFILE="$profile" MANIFEST="$manifest" python3 - <<'PY'
...
obj={'schema':'KMDSB.W04.M21.LSamplingPairLane.v0.1','protocol':'protocol/W04_M21_G2B_TRANSFER_L_SAMPLING_DIRECTION_INTERACTION_AUDIT_v0.1.md','pair':os.environ['PAIR'],'l_logstep':m['l_logstep'],'l_linstep':m['l_linstep'],'provider_head':head,'exact_head':head==os.environ['PIN_ENV'],'case_rc':status,'all_cases_rc0':set(status)=={'ref','f2','f3','f4'} and all(v==0 for v in status.values()),'ini_sha256':{c:H('m21_cases/'+c+'.ini') for c in ('ref','f2','f3','f4')},'cl_permille_sha256':H('class/cl_permille.pre'),'ncdm_tight_sha256':H('verification/m21/m21_ncdm_tight.pre'),'profile_sha256':H(os.environ['PROFILE']),'manifest_sha256':H(os.environ['MANIFEST']),'varied_keys':m['varied_keys'],'varied_key_count':m['varied_key_count'],'duplicate_free_serialization':m['duplicate_free_serialization']}
