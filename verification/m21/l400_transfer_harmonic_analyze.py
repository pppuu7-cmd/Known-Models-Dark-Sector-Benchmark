...
def diag(path:Path)->dict[int,np.ndarray]:
 ...
 for lv in sorted(set(int(round(x)) for x in a[:,0])):
  ...
  if np.max(np.abs(b[:,8]-b[0,8]))>1e-25*max(abs(b[0,8]),1.0): raise RuntimeError(f'cl column not constant over q for l={lv}')
  out[lv]=b
 return out
...
def main(pair_id:str,newroot:Path,parentroot:Path,meta_path:Path,patch_path:Path,outp:Path)->int:
 ...
 if not all(checks.values()):
  obj={'schema':'KMDSB.W04.M21.L400TransferHarmonicLane.v0.1','protocol':PROTOCOL,'pair_id':pair_id,'lane':cfg['name'],'classification':'LANE_BLOCKED','checks':checks,'null_cl_l2':null,'K1_promoted':False,'K3_promoted':False,'K4_promoted':False,'physical_falsification':False}
 ...
 for c in CASES:
  ...
  for l in sorted(cfg['required']):
   dv=float(ds[c][l][0,8]); cv=cl_ee_at(newroot,c,l); err=abs(dv-cv); cl_diag_abs[c][str(l)]=err
   cl_diag_match &= err <= 1e-12*max(abs(dv),abs(cv),1e-300)
 checks['diag_cl_matches_output']=bool(cl_diag_match)
 if not cl_diag_match:
  obj={'schema':'KMDSB.W04.M21.L400TransferHarmonicLane.v0.1','protocol':PROTOCOL,'pair_id':pair_id,'lane':cfg['name'],'classification':'LANE_BLOCKED','checks':checks,'diag_cl_abs_error':cl_diag_abs,'null_cl_l2':null,'K1_promoted':False,'K3_promoted':False,'K4_promoted':False,'physical_falsification':False}
  outp.write_text(json.dumps(obj,indent=2,sort_keys=True)+'\n'); return 1
 ...
 obj={'schema':'KMDSB.W04.M21.L400TransferHarmonicLane.v0.1','protocol':PROTOCOL,'provider':f'lesgourg/class_public@{PIN}','pair_id':pair_id,'lane':cfg['name'],'neighbors':cfg['neighbors'],'checks':checks,'null_cl_l2':null,'null_max':float(max(null.values())),'transfer':transfer,'ee_integrand':integ,'transfer_locally_l400_specific':bool(transfer_local),'integrand_locally_l400_specific':bool(integ_local),'direct_sparse_cl_l400_response':dc,'direct_sparse_cl_l400_E':float(Ecl),'classification':cls,'K1_promoted':False,'K3_promoted':False,'K4_promoted':False,'physical_falsification':False}
 outp.write_text(json.dumps(obj,indent=2,sort_keys=True)+'\n')
 print(json.dumps({'pair_id':pair_id,'classification':cls,'R_null_max':obj['null_max'],'Ecl400':Ecl,'E_transfer':{str(l):transfer[l]['E'] for l in sorted(transfer)},'E_integrand':{str(l):integ[l]['E'] for l in sorted(integ)}},indent=2,sort_keys=True))
 return rc
if __name__=='__main__':
 if len(sys.argv)!=7: raise SystemExit('usage: l400_transfer_harmonic_analyze.py PAIR_ID NEW_ROOT PARENT_ROOT LANE_META PATCH_MANIFEST OUT.json')
 raise SystemExit(main(sys.argv[1],Path(sys.argv[2]),Path(sys.argv[3]),Path(sys.argv[4]),Path(sys.argv[5]),Path(sys.argv[6])))
