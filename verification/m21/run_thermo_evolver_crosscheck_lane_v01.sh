...
lane="$1"; PIN='e85808324f51fc694d12e3ed7439552a3c3f9540'; profile="thermo_evolver_${lane}.pre"; manifest="thermo_evolver_${lane}_manifest.json"
...
python3 verification/m21/build_thermo_evolver_crosscheck_profile.py "$lane" class/cl_permille.pre verification/m21/m21_ncdm_tight.pre "$profile" "$manifest"
...
obj={'schema':'KMDSB.W04.M21.ThermoEvolverCrosscheckLane.v0.1','protocol':'protocol/W04_M21_CONDITIONAL_THERMO_EVOLVER_CROSSCHECK_v0.1.md','lane':os.environ['LANE'],'thermo_evolver':m['thermo_evolver'],'tol_thermo_integration':m['tol_thermo_integration'],'generic_evolver':m['generic_evolver'],'provider_head':head,'exact_head':head==os.environ['PIN_ENV'],'case_rc':status,'all_cases_rc0':set(status)=={'ref','f2','f3','f4'} and all(v==0 for v in status.values()),'ini_sha256':{c:H('m21_cases/'+c+'.ini') for c in ('ref','f2','f3','f4')},'cl_permille_sha256':H('class/cl_permille.pre'),'ncdm_tight_sha256':H('verification/m21/m21_ncdm_tight.pre'),'profile_sha256':H(os.environ['PROFILE']),'manifest_sha256':H(os.environ['MANIFEST']),'varied_keys':m['varied_keys'],'varied_key_count':m['varied_key_count'],'duplicate_free_serialization':m['duplicate_free_serialization']}
pathlib.Path('lane_meta.json').write_text(json.dumps(obj,indent=2,sort_keys=True)+'\n')
assert obj['exact_head'] and obj['generic_evolver']=='0' and obj['varied_keys']==['thermo_evolver','tol_thermo_integration'] and obj['varied_key_count']==2 and obj['duplicate_free_serialization']
PY
exit "$any_fail"
