...
!above calculate derivative of dimensionless axion density'''
    s=replace_once(s,old2,new2,'recfast zero derivative close')
    rec.write_text(s)
...
def analyze(patched:Path, original:Path, statusp:Path, out:Path):
    ...
    res={'schema':'KMDSB.M19.axionCAMB2.zeroBypassRecovery.v1','provider_pin':'891e779cc0bd422e49f97533e6c2fc761149737d','tolerance':TOL,'status':st,'physical_falsification':False,'scientific_promotion':{'K1':False,'K2_K9':False},'preregistration':'protocol/W04_M19_AXIONCAMB2_ZERO_BYPASS_RECOVERY_PREREGISTRATION_v0.1.md'}
    if st.get('build_orig')!=0 or st.get('build_patch')!=0:
        res['classification']='M19_ZERO_BYPASS_RECOVERY_OUTPUT_BLOCKED'; res['reason']='build'; out.write_text(json.dumps(res,indent=2,sort_keys=True)+'\n'); return
    ...
    if not contracts['zf'] or not contracts['zd']:
        cls='M19_ZERO_BYPASS_RECOVERY_EXECUTION_BLOCKED'
    elif not contracts['pp'] or not contracts['po']:
        cls='M19_ZERO_BYPASS_RECOVERY_OUTPUT_BLOCKED'
    else:
        ...
        if not pok: cls='M19_ZERO_BYPASS_REJECTED_FINITE_PATH_CHANGED'
        elif not zok: cls='M19_ZERO_BYPASS_RECOVERY_IDENTITY_BLOCKED'
        elif st.get('debug_zf')!=0: cls='M19_ZERO_BYPASS_RECOVERY_OUTPUT_BLOCKED'
        else: cls='M19_AXIONCAMB2_ZERO_BYPASS_RECOVERY_PASS_WITH_SCOPE'
    res['classification']=cls
    res['next_if_pass']='separately preregister historical pure-CAMB CDM comparator; do not promote K1 from recovery alone'
