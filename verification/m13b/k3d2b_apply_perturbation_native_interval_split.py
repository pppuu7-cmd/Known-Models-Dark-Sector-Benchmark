...
def main() -> int:
    ap=argparse.ArgumentParser(); ap.add_argument('root'); ap.add_argument('--manifest'); a=ap.parse_args()
    ...
    # 1) boundary ownership: only the two perturbation qfield handoff guards.
    ...
    # 2) insert a native approximation interval after CLASS has built its own interval arrays.
    ...
    block=r'''  free(interval_number_of);
  ...
  /** - fill the structure containing all fixed parameters, indices'''
    ...
    # 3) evolver lower bound only: vector_init remains at exact interval boundary.
    ...
    if s.count('nextafter(tau_handoff_kmdsb,interval_limit[index_interval+1])') != 1: raise RuntimeError('ULP seam missing')
    p.write_text(s)
    manifest={'schema':'KMDSB.W03.M13b.K3D2BPerturbationNativeIntervalSplitPatch.v0.1','changed_files':['source/perturbations.c'],'conditional_qfields':True,'tau_from_background_z5':True,'duplicates_native_approximation_row':True,'state_vector_unchanged':True,'vector_init_at_exact_boundary':True,'evolver_post_handoff_start_one_ulp':True,'perturbation_boundary_guard_replacements':2,'changes_tolerances':False,'changes_solver_family':False,'changes_boltzmann_hierarchy_equations':False,'changes_einstein_sources':False,'sha256_before':sha(before),'sha256_after':sha(s)}
