...
Updated: 2026-09-10
Purpose: resume KMDSB from repository evidence, not chat memory.

## Authority rule
Repository evidence and validated Actions/artifacts override chat summaries. Historical evidence is never silently rebased. W00-W02 DSIR authority: `e3276e2193f6a5200b541a194e3175356ae5a1c1`; W03 start/M07: `328f2ca80b724870b851c7fe6366cce1ca5086cd`; observation-method overlay from M08: `864952e1520d82473a9e976edfeb69f9899d174d`; later M09/M10 overlay inspected: `bc28acc47cc5facba046741fd09f710ae8da9689`.

## Restore order
Read `recovery/STATE.md`, this file, required-properties protocol, census, mandatory-properties matrix, wave protocol, future-model methodology, benchmark/design ledgers, research log, then active wave/model files and current Actions.

## Mission
Pass all maintained response-distinct known cosmological mechanism families through K0-K9/B0-B9 before deciding whether existing/adapted/hybrid/new physics is required. `BLOCKED`, `NONIDENTIFIABLE`, pathology and incomplete coverage are not physical falsification. Undefined channels mask, never zero-impute.

## Current waves
W00 COMPLETE; W01 COMPLETE; W02 COMPLETE; W03 ACTIVE; W04 DM PLANNED; W05 MG PLANNED; W06 unified/geometry/adversarial PLANNED; W07 rigidity PLANNED; W08 true holdout PLANNED; W09+ escape/literature-tail iterative.

## W03 essential evidence
- M07 canonical quintessence: reference/quotient clean, within-family support, but full CPL absorbs almost all late-time response; common ShapeFit control is observationally tiny.
- M08 CPL: full 2D smooth-DE comparator; parameter count != identified rank.
- M09 native CLASS EDE: terminal `BLOCKED_IMPLEMENTATION`, not EDE falsification.
- M10 scalar EDE: scoped theory-response survivor; exact common observation bridge remains open.
- M11 effective sound-speed response: step-stable and outside local CPL theory span (~92.56% combined residual) but very weak absolute secondary rank; covariant validation required.
- M12 smooth phantom: represented by the same local constant-w line with opposite orientation; minimal wrong-sign ghost issue remains separate.
- M13a PPF/CPL crossing: represented by M08 with scope; true covariant multifield M13b remains open.

## M11b CLASS_GSF terminal provider-specific blocker
Pinned `KunhaoZhong/CLASS_GSF@07e015246c4b40f4e22bb50c9a0a63a621bb61f7`, model 6.

Run `34429080377`: build and LCDM pass, all model-6 GSF cases fail. Frozen nonzero kinetic-seed recovery run `34429876130`: all 16 GSF diagnostics fail while LCDM passes. Provider control run `34429997792`: exact source builds and the unmodified committed model-1 `dgf.ini` produces fresh outputs. No author-supplied working model-6 example was found at the pin.

Classification: `BLOCKED_IMPLEMENTATION_PROVENANCE` for that branch/provider only. Do not reopen arbitrary initial-condition tuning. It is not physical k-essence falsification.

## Scherrer provider audit — important correction
Pinned `Eladio-Moreno/k-essence-dynamics@f3f010e1ed74c86ce6a431a435fa93988f749ee2`, `Cuadratico/`.

Recovered provider-control run `34430793576` is executable; the earlier apparent failure was a harness output-prefix mistake because author `hi.ini` uses `root=output/test_`.

Source implements `G2=-F0+F2*(X-X0)^2-0.5*m_phi^2 phi^2`, but the committed author example is **not** unified DM+DE: `Omega_Lambda=0.69`, `Omega_cdm=0`, scalar matter target `DM_schm=0.26`, and source stores `pba->f0_schm=0`. Separate Lambda provides acceleration.

Classification: `REPRESENTATIVE_MISMATCH_NO_M15_PROMOTION`.

Therefore:
- do not use this provider/example to promote M15 unified dark fluid;
- do not use it as M11 DE-only covariant validation;
- provider execution is infrastructure evidence only;
- this is not a physical failure of Scherrer/GCG/unified-dark-sector physics.

Authority: `models/scherrer_unified_dark_sector/representative_scope_audit.md`, `waves/wave_03_expanded_dark_energy/M15_SCHERRER_REPRESENTATIVE_SCOPE_RESULT.json`.
...
- F14/M14: `K0_PASS_WITH_SCOPE_K1_SOURCE_AUDIT_REQUIRED`. Root `iDM.ini` is absent, but a tracked author-generated workload exists at `benchmark/gcc/tmp_ini/pgo_hyperbolic_cmb.ini`. Preregistered run `34439614496` builds and executes it with fresh Cl/P(k) products after root-only plumbing. Do not call this K1: its q1-q4 are zero while `cdm_c=0.1`; source-exact full decoupling remains to be frozen.
...
1. M14 provider K0 is recovered with scope through run `34439614496`; next source-audit all q/exp and `cdm_c` coupling paths and freeze the exact total decoupling map.
2. Prospectively preregister K1 only after that map is unambiguous; compare the decoupled branch to the correct uncoupled scalar+CDM comparator in the same solver.
3. Provider PASS does not equal K1-K9 PASS; only after K1 may physical nonzero-coupling grids be defined.
...
### 2026-09-10 — M14 LisaGoh/CDE branch audit delta
Run `34490297354` split the independent exact-reference patch into R1/R2. Only R2 is noninterfering, but R2 cannot execute the exact full reference; R1 enables exact execution only by changing nonsingular outputs and is therefore inadmissible. Alternate author branch `7bin@2a572b39d4ae0a3c940b6f179585686ae1a03881` contains the same problematic zero-state structures and supplies no documented exact-reference prescription. Keep exact background reference valid but full perturbation K1 provider-blocked; do not promote the KMDSB verification patch to K1/K3 or reopen K4/K5. Next gate is a different independent source-complete coupled-quintessence provider.
...
## 2026-09-13 — M13b K3B equation-authority boundary localized
- K3B0 canonical classification: `M13B_K3B0_PUBLISHED_EINSTEIN_TRACE_FORM_NOT_LITERAL_IMPLEMENTATION_AUTHORITY`; run `34780923216`, artifact `10325456725`, digest `sha256:a32d1eecd00495af2b57ed0f723630a6520b0fcc266526bf0618663864eb459d`. In the frozen pressureless matter-era test, the standard Newtonian-gauge trace admits constant nonzero Phi exactly while the literal printed trace leaves `-8 Phi/tau^2`; the printed physical-pressure RHS prefactor is one third of the standard trace coefficient.
- K3B1 canonical classification: `M13B_K3B1_PUBLISHED_PERTURBED_KG_NOT_LITERAL_COVARIANT_IMPLEMENTATION_AUTHORITY`; run `34781190138`, artifact `10325052833`, digest `sha256:ca34c92c7e64e71fdaf06f0c06d1fe97ababfeeec8714b60bf928ef5b5661165`. Literal-minus-covariant differences are `phip*Phi_prime - 2 a^2 V_phi Phi` and `psip*Phi_prime + 2 a^2 V_psi Phi`; exact canonical/phantom mass-term signs pass.
...
### F15 provider discovery v0.1 reviewed — 2026-09-14
Run 34826008004 / artifact 10340128264 yielded no source-complete NGCG provider after manual review. Sole machine provenance lead was unrelated prose/reference code and was rejected. Classification `F15_EXACT_NGCG_DISCOVERY_NO_SOURCE_COMPLETE_PROVIDER_AFTER_SOURCE_REVIEW_2026_09_14`; K0/K1 unchanged; no physical failure. Continue only prospectively frozen author/equation keyed provider search or another independent implementation lead.
